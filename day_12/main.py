...
async def on_message(message: cl.Message):
    ...
    use_mcp = chat_settings.get("use_mcp", "Включено")

    # Show typing indicator
    await response_placeholder.stream_token("🤔 Думаю...")

    # Get MCP tools and format them for the system prompt (only if MCP is enabled)
    mcp_tools = await get_mcp_tools() if use_mcp == "Включено" else None
    tools_description = ""
    ...
    # Check if user wants to use MCP tool (only if MCP is enabled)
    should_use_mcp = False
    mcp_result = None
    if use_mcp == "Включено" and mcp_session and mcp_tools and mcp_tools.tools:
    ...
    if not api_key or not folder_id:
        ...
        try:
            response = await provider.completions(
                messages=history, temperature=temperature, model=model, tools=mcp_tools.tools if mcp_tools else None
            )
            ...
            if response and response.text:
                ...
                if "Tool calls requested:" in response.text:
                    # Extract tool call information
                    # For now, we'll just show that tool calls were requested
                    # In a more complete implementation, we would actually call the tools
                    # and then make another API call with the tool results
                    await response_placeholder.stream_token("🔧 Обнаружены вызовы инструментов. Обработка...")
                    await response_placeholder.update()
                else:
