...
def _parse_json(text: str):
    try:
        ...
        # Convert each item to ResponseMessage
        messages = []
        for item in data:
            if isinstance(item, dict) and 'id' in item and 'text' in item:
                messages.append(ResponseMessage(id=item['id'], text=item['text']))
            elif isinstance(item, dict) and 'text' in item:
                # If no id is provided, use 0 as default
                messages.append(ResponseMessage(id=0, text=item['text']))
...
async def on_message(message: cl.Message):
    ...
    model = cl.user_session.get("model", PROVIDER_CONFIGS["Ollama"]["default_model"])
